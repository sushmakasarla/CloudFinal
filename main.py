...
def connecttoDataBase():
    config = {
    'host': 'cloudcomputingserver.mysql.database.azure.com',
    'port':'3306',
    'user': 'cloudadmin',
    'password':'Samsung@26',
    'database':'clouddb'
...
""")
    ...
    CREATE TABLE IF NOT EXISTS products (
        ...
        BRAND_TY varchar(255),
        NATURAL_ORGANIC_FLAG varchar(255),
...
def readCSVFileandStoretoAzure(csvfilename,dataFrom):
    ...
    df = pd.read_csv("/Users/sushmakasarla/Downloads/Others/8451_The_Complete_Journey_2_Sample/400_products.csv")
    df.columns = df.columns.str.replace(' ', '')
    df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
    dftotuple = list(df.to_records(index=False))
    for eachtuple in dftotuple:
...
def loadDashboard():
    ...
    Fourthdata = executeselectquery(
        "Select AVG(SPEND) as spend,YEAR as year from transactions as a group by a.YEAR;");
